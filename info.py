...
auth_req_channel = environ.get('AUTH_REQ_CHANNEL', '-1002456267845 -1002695471786 -1002568854373')  # requst to join Channel for force sub (make sure bot is admin) only for bot ADMINS  
AUTH_CHANNELS = [int(channels_id) for channels_id in environ.get('AUTH_CHANNELS', '-1002568854373').split() if re.match(r'^-?\d+$', channels_id)]  # Channels for force sub (make sure bot is admin)
...
if 'DYNO' in environ:
    ...
    ON_HEROKU = False
URL = environ.get("FQDN", "https://spotty-nona-deendayaldhakad12-6fa4b35a.koyeb.app/")
